...
import { apiFetch, fetchGapStats } from "./api";
...
export async function fetchAnalyticsData(userId) {
  ...
  // ── Step 4: fetch gap stats ───────────────────────────────────────────────
  let gapStats = null;
  try {
    gapStats = await fetchGapStats(userId);
  } catch {
    // Gap stats unavailable — non-critical, fail silently
  }

  return { data, summary: summaryData, yearTotals, monthTotals, gapStats };
}
