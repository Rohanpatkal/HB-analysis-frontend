...
/**
 * Gap frequency stats — top-5 smoke-free gap lengths per year and globally.
 * Sorted by occurrence count descending (most frequent gap first).
 * Returns {
 *   globalGaps: { average, top5: [{ gap, count }] },
 *   yearlyGaps: { "2024": { average, top5: [...] }, ... }
 * }
 */
export async function fetchGapStats(userId) {
  const { data } = await apiFetch(`/stats/${userId}/gaps`);
  return data;
}
 ...
 * Returns { success, comments: [...], total }
 */
export async function fetchComments({ limit = 50, skip = 0 } = {}) {
  return apiFetch(`/comments?limit=${limit}&skip=${skip}`);
}
 ...
 */
export async function postComment({ userId, userName, text }) {
